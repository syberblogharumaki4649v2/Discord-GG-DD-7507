...
async def on_message(message):
        ...
        if message.content == 'spb!re':
            ...
            if setmsg == 812275752904163358:
                ...
                ch = client.get_channel(947469205538762812)
                await ch.send("`ログ機能を再起動＆を更新します`")
                game = discord.Game(f'ログ機能を再起動中です....')
                await client.change_presence(activity=discord.Game(name=game))
                proc = subprocess.Popen(['python3', '3.py'])
                await client.logout()
                proc.poll()
                exit
        ...
        if message.content == 'spb!down':
            ...
            if setmsg == 812275752904163358:
                ch = client.get_channel(947469205538762812)
                await ch.send("`ログ機能をシャットダウンします`")
                proc = subprocess.Popen(['python3', 'down2.py'])
                await client.logout()
                proc.poll()
                exit
