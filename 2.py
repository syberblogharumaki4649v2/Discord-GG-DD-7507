...
try:
    ...
    file.close()
global_channel_name = "hrk_グローバルチャット" #設定したいチャンネル名を入力
...
async def on_message(message):
    ...
    if message.content == 'spb!re':
        ...
        if setmsg == 812275752904163358:
            ...
            ch = client.get_channel(947469205538762812)
            await ch.send("`グローバル機能を再起動＆を更新します`")
    ...
    if message.content == 'spb!down':
        ...
        if setmsg == 812275752904163358:
            ch = client.get_channel(947469205538762812)
            await ch.send("`グローバル機能をシャットダウンします`")
            proc = subprocess.Popen(['python3', 'down1.py'])
            await client.logout()
            proc.poll()
            exit
